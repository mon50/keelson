...
describe('implementation workflow cleanup', () => {
  it('removes the standalone verify skill from core, Claude Code, and Codex installs', () => {
    expect(fs.existsSync(path.join(repoRoot, 'skills/core/reforge-verify'))).toBe(false);
    expect(fs.existsSync(path.join(repoRoot, '.claude/skills/reforge-verify/SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(repoRoot, '.agents/skills/reforge-verify/SKILL.md'))).toBe(false);
  });

  it('keeps verification inside the task implementation loop', () => {
    const impl = read('skills/core/reforge-impl/SKILL.md');
