...
import { createKeelsonServer } from '../src/server';
...
async function makeWorkspace(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'keelson-server-'));
  await mkdir(join(cwd, '.keelson/specs/daily-report'), { recursive: true });
  await writeFile(
    join(cwd, '.keelson/specs/daily-report/spec.json'),
...
describe('HttpServer', () => {
  it('serves the rendered confirmation UI and 404s unknown routes', async () => {
    ...
    const server = createKeelsonServer();
    servers.push(server);
    ...
    expect(root.headers.get('x-keelson-render-mode')).toBe('spec-preview');
  ...
  it('serves a locked prototype artifact when uiArtifacts.prototype is set', async () => {
    const cwd = await makeWorkspace();
    await writeFile(
      join(cwd, '.keelson/specs/daily-report/prototype.html'),
    ...
    expect(root.status).toBe(200);
    expect(root.headers.get('x-keelson-render-mode')).toBe('prototype-artifact');
    expect(root.headers.get('x-keelson-prototype-artifact')).toBe(
      '.keelson/specs/daily-report/prototype.html'
    );
  ...
  it('percent-encodes prototype artifact path headers for non-Latin-1 paths', async () => {
    ...
    await mkdir(join(cwd, '.keelson/specs', specName), { recursive: true });
    await writeFile(
      join(cwd, '.keelson/specs', specName, fileName),
      '<!doctype html><html><body><main>Locked Prototype</main></body></html>'
    );
    await writeFile(
      join(cwd, '.keelson/specs', specName, 'spec.json'),
      JSON.stringify({
        ...dailyReportSpec(),
        uiArtifacts: {
          mode: 'locked',
    ...
    const root = await fetch(`${address.url}/`);
    const expectedPath = `.keelson/specs/${encodeURIComponent(specName)}/${encodeURIComponent(
      fileName
    )}`;

    expect(root.status).toBe(200);
    expect(root.headers.get('x-keelson-prototype-artifact')).toBe(expectedPath);
    expect(root.headers.get('x-keelson-artifact-path')).toBe(expectedPath);
    expect(await root.text()).toContain('Locked Prototype');
  });
  ...
  it('rejects a locked prototype artifact when the approved digest does not match', async () => {
    ...
    await writeFile(join(cwd, '.keelson/specs/daily-report/prototype.html'), artifactHtml);
    await writeFile(
      join(cwd, '.keelson/specs/daily-report/spec.json'),
      JSON.stringify({
        ...dailyReportSpec(),
...
describe('FileWatcher', () => {
  it('notifies once for a saved file and stops cleanly', async () => {
    const cwd = await makeWorkspace();
    const specPath = join(cwd, '.keelson/specs/daily-report/spec.json');
