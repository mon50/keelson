...
async function makeWorkspace(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'keelson-loader-'));
  tempDirs.push(dir);
  await mkdir(join(dir, '.keelson'));
  return dir;
}

async function writeNamedSpec(cwd: string, name: string, value: unknown): Promise<void> {
  await mkdir(join(cwd, '.keelson/specs', name), { recursive: true });
  await writeFile(join(cwd, '.keelson/specs', name, 'spec.json'), JSON.stringify(value));
}

afterEach(async () => {
  for (const dir of tempDirs.splice(0)) {
    await chmod(join(dir, '.keelson'), 0o700).catch(() => undefined);
  }
});

describe('loadSpec', () => {
  it('loads the only named spec from .keelson/specs', async () => {
    ...
    if (result.ok) {
      expect(result.specPath).toBe('.keelson/specs/daily-report/spec.json');
    }
  });
  ...
  it('falls back to legacy .keelson/spec.json when no named specs exist', async () => {
    const cwd = await makeWorkspace();
    await writeFile(
      join(cwd, '.keelson/spec.json'),
  ...
  it('reports NOT_FOUND for a missing spec file', async () => {
    ...
    if (!result.ok) {
      expect(result.error.message).toContain('/keelson-init');
    }
  });

  it('reports PARSE_ERROR with the parser message for invalid JSON', async () => {
    const cwd = await makeWorkspace();
    await mkdir(join(cwd, '.keelson/specs/invalid-spec'), { recursive: true });
    await writeFile(join(cwd, '.keelson/specs/invalid-spec/spec.json'), '{ invalid');
