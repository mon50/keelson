...
import { createKeelsonServer } from './server';
import { createWatcher } from './watcher';

export async function main(argv = process.argv.slice(2), cwd = process.cwd()): Promise<void> {
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log('Usage: keelson-render [--port <port>] [--spec <name-or-path>]');
    console.log('Starts a local specification confirmation UI from .keelson/specs/<name>/spec.json.');
    return;
  }
  ...
  console.log(`Keelson renderer running at ${address.url}`);
...
function readPort(argv: string[]): number {
  ...
  return Number(process.env.KEELSON_RENDER_PORT || process.env.PORT || 4317);
}
...
export { createKeelsonServer } from './server';
