...
import type { LoadError, LoadResult, KeelsonSpec } from './types';

export const LEGACY_SPEC_RELATIVE_PATH = '.keelson/spec.json';
export const SPECS_RELATIVE_DIR = '.keelson/specs';
...
export async function loadSpec(cwd: string, specArg?: string): Promise<LoadResult> {
  ...
  try {
    ...
    if (nodeError.code === 'ENOENT') {
      return {
        ...
        error: {
          code: 'NOT_FOUND',
          message: `${resolved.value.relativePath} が見つかりません。先に /keelson-init を実行してください。`
        }
      };
    }
    return {
  ...
  try {
    return {
      ok: true,
      spec: JSON.parse(raw) as KeelsonSpec,
