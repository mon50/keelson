---
name: reforge-impl
description: Implement one entity from the approved Reforge specification (DB migration + API endpoint + UI component + tests). Reads tasks.json to identify the next pending task. Gates on spec.json existence, meta.approved=true, tasks.json existence, tech completeness, and entity definition completeness.
...
`spec.context.mode` が `"brownfield"` の場合、既存リポジトリへの新機能追加として扱う。実装の自由度は `spec.json` に記録された変更範囲と既存コードから読める規約に制限する。

1. `spec.context.changeScope` を読み取り、対象 feature、`affectedAreas`、`allowedWriteAreas`、`protectedAreas` を確認する。
   - `allowedWriteAreas` が空でない場合、DB/API/UI/test の生成先はその範囲内に限定する。
   - `protectedAreas` に含まれるパス、ドメイン、機能名に対しては編集しない。
   - 必要な編集が `allowedWriteAreas` の外に出る場合は、実装を開始せず `blocked` として報告し、`/reforge-update` で変更範囲を明示するよう案内する。
2. 既存コードから命名規則、ディレクトリ構成、ルーティング規約、テスト配置が読み取れる場合はそれに合わせる。ただし、既存コードにない仕様や業務ルールを推測して追加しない。
3. `spec.context.repository.conventions` が存在する場合は、DB/API/UI/test の生成方式より優先して尊重する。
4. `spec.context.acceptanceCriteria` が存在する場合は、実装後の報告に各項目がどのファイルまたはテストで満たされたかを含める。自動確認できない項目は「manual check required」として残す。
5. `spec.context.risks` が存在する場合は、完了報告に残リスクとして記載する。リスクを実装で勝手に解消したことにしない。

Brownfield でも、実装の source of truth は `SPEC_PATH` である。既存リポジトリは規約と制約を読むための文脈であり、spec にない product decision を補う根拠にはしない。
...
3. 開始前確認として `.reforge/specs/<name>/tasks.json` (`TASKS_PATH`) を再読み取りし、対象entityタスクが `status: "in_progress"` になっていることを確認する。
...
4. ロールバック後確認として `.reforge/specs/<name>/tasks.json` (`TASKS_PATH`) を再読み取りし、対象entityタスクが `status: "pending"` になっていることを確認する。
...
4. 完了後確認として `.reforge/specs/<name>/tasks.json` (`TASKS_PATH`) を再読み取りし、対象entityタスクが `status: "done"` になっていることを確認する。
