---
name: reforge-render
description: Start a local HTML prototype server from .reforge/specs/<name>/spec.json and run the approval flow that writes meta.approved to spec.json.
...
1. **Installed project**: `node .reforge/server/index.js --spec <name>` — used when the project has installed Reforge via npm
2. **Source development**: `node reforge-renderer/dist/index.js --spec <name>` — used when running from the Reforge source repository
...
Use `Bash: test -f .reforge/server/index.js` and `Bash: test -f reforge-renderer/dist/index.js` to check which path exists before launching. If only one spec exists, `--spec <name>` may be omitted; if multiple specs exist, pass the resolved spec name.
...
`Read QUESTIONS_PATH` が存在すれば読み込む。
...
エントリポイントが見つかった場合:
- `Bash: node <entry_point> --spec <name>` でサーバーを起動する（デフォルトポート 4317、または `--port <number>` で指定）。
...
**ユーザーが承認した場合**:
1. `Read SPEC_PATH` を再読み込みして最新の内容を取得する。
2. `meta.approved` を `true` に更新する。
3. `meta.approvedAt` に現在時刻の ISO 文字列を記録する。
4. `meta.approvedDigest` に承認時点の spec 内容を識別できる digest または timestamp marker を記録する。
5. 他のフィールドを保持して `Write SPEC_PATH` で書き込む。
6. `Bash: サーバー停止` を実行してサーバーを終了する。
7. 次のステップを案内する:「承認が完了しました。`/reforge-plan` を実行してタスクキューを生成してください。」
...
- Written spec preserves all existing top-level keys (`meta`, `entities`, `views`, `flows`, `tech`, optional `requirements`, optional `context`).
- Only `meta.approved`, `meta.approvedAt`, and `meta.approvedDigest` are changed; no other fields are modified.
