...
```
/reforge-requirements "Add team invitations to this existing SaaS repo. Follow existing auth, email, and team settings conventions."
/reforge-us
/reforge-design
/reforge-proto
/reforge-plan
...
```
/reforge-requirements "Add CSV export to the existing admin reports area. Do not touch billing or auth. Use the current table and filter conventions."
```

Avoid vague prompts:

```
/reforge-requirements "Reverse engineer this app"
```
...
After approval, `/reforge-impl` reads the approved artifact bundle and `plan.md`.
...
Verification is task-local. `/reforge-impl` runs the relevant checks for the selected task, updates `plan.md`, and reports any manual verification that remains.

## Common Failure

**Symptom:** The agent wants to change unrelated parts of the repository.

**Fix:** Add explicit `Files Not To Touch` or narrower `Files To Touch` in `/reforge-design`, then re-run `/reforge-proto` and `/reforge-plan`.

**Symptom:** Reforge cannot infer existing stack or conventions.

**Fix:** Answer the design question explicitly or point Reforge at the relevant files. Reforge should not guess the stack from weak evidence.

**Symptom:** The spec describes the whole product instead of one feature.

**Fix:** Re-run `/reforge-requirements` with a feature-scoped description, then rebuild downstream artifacts.
